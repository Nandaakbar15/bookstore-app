...
export default function LoginPages() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // 1. Ambil token dan object user dari response.data
      const token = response.data.token;
      const user = response.data.user; // 👈 Deklarasikan variabel 'user' di sini

      // 2. Simpan ke localStorage menggunakan object 'user'
