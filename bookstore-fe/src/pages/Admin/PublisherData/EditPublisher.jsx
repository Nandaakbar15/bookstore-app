...
export default function EditPublisherPages() {
  const { id } = useParams();
    ...
    const getPublisherById = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/admin/getPublisherById/${id}`,
        );
        const { name, address, phone } = res.data.data;
  ...
  const editPublisher = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:3000/api/admin/updatePublisher/${id}`,
        {
          name: name,
          address: address,
          phone: phone,
        ...
        <main className="grow">
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="mx-auto mt-10 max-w">
              <Card>
                <CardContent>
                  <form className="space-x-4 p-6" onSubmit={editPublisher}>
                    <div class="mb-5">
                      <label
                        for="name"
