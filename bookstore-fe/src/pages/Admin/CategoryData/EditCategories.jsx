...
export default function EditCategoryPages() {
  const { id } = useParams();
  ...
  const editCategories = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:3000/api/admin/updateCategories/${id}`,
        {
          name: name,
        },
      );
    ...
    const getCategoriesById = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/admin/getCategoriesById/${id}`,
        );
        ...
        <main className="grow">
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="mx-auto mt-10 max-w">
              <Card>
                <CardContent>
                  <form className="space-x-4 p-6" onSubmit={editCategories}>
                    <div className="mb-5">
                      <label
                        htmlFor="name"
                    ...
                    <button
                      type="submit"
                      className="text-white bg-blue-500 inline-block hover:bg-blue-700 rounded-lg shadow-lg px-4 py-2"
                    >
                      Edit!
                    </button>
                  </form>
