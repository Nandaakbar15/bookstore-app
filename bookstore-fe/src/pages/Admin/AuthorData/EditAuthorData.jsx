...
export default function EditAuthorsPages() {
  const { id } = useParams();
  ...
  const editAuthors = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(
        `http://localhost:3000/api/admin/updateAuthor/${id}`,
        {
          name: name,
        },
      );
    ...
    const getAuthorById = async () => {
      try {
        const res = await axios.get(
          `http://localhost:3000/api/admin/getAuthorById/${id}`,
        );
        ...
        <main className="grow">
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            ...
            <div className="mx-auto mt-10 max-w">
              <Card>
                <CardContent>
                  <form className="space-x-4 p-6" onSubmit={editAuthors}>
                    <div class="mb-5">
                      <label
                        for="name"
                        class="block mb-2.5 text-sm font-medium text-heading"
                      >
                        Authors Name <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        id="name"
